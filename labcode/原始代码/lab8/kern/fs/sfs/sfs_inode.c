...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2310364 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
	...
	*/
    ...
    // (1) 如果offset没有对齐到块边界，处理第一个不完整的块
    blkoff = offset % SFS_BLKSIZE;  // offset在块内的偏移
    if (blkoff != 0) {
        // 计算第一块需要读写的大小
    ...
    // (3) 如果结束位置没有对齐到块边界，处理最后一个不完整的块
    // 注意：只有当 alen < (endpos - offset) 时才需要处理尾部
